...
def getFileModelForIPA(ipapath):
    ...
    extractdir = os.path.dirname(ipapath)+'/'+basename# join(os.path.dirname(ipapath),basename)
    os.mkdir(extractdir)
	
    for zipinfo in zip.filelist:
		is_dir = zipinfo.filename.endswith('/')
		filename = zipinfo.filename.decode('utf-8').encode('gbk')
		#print filename
		dstfilename = extractdir+'/'+zipinfo.filename
		if is_dir and not os.path.exists(dstfilename.decode('utf-8').encode('gbk')):
			#print zipinfo.filename
			os.mkdir(dstfilename.decode('utf-8').encode('gbk'))
		else:
			with open(dstfilename.decode('utf-8').encode('gbk'), 'wb') as output_file:
				shutil.copyfileobj(zip.open(zipinfo.filename), output_file)
			#print zipinfo.filename + '   not dir'
    appdir = ''
    appname = ''
    ...
    for i,tup in enumerate(resultTuple):
        ...
        for i,item in enumerate(tup[1]):
            if item.sizeChange > sizelimit:
                ...
                if hasRecursiveComparation:
                    output.writeHtml('<tr><td rowspan="2">%s</td><td align="right">%s</td></tr>' % (item.name.decode('gbk').encode('utf-8'),binarySize(item.sizeChange)[1]))
                else:
                    output.writeHtml('<tr><td>%s</td><td align="right">%s</td></tr>' % (item.name.decode('gbk').encode('utf-8'),binarySize(item.sizeChange)[1]))
